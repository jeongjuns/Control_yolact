...
#from backbone import construct_backbone, Bottleneck
from backbone import*
import torch.backends.cudnn as cudnn
from utils import timer
from utils.functions import MovingAverage, make_net
from W_conv import bbox_Conv2d, conf_Conv2d, mask_Conv2d, seg_Conv2d, fpn_lat_layers_Conv2d,fpn_pred_layers_Conv2d, fpn_down_layers_Conv2d
...
class PredictionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0):
        ...
        if parent is None:
            ...
            """
            self.bbox_layer = bbox_Conv2d(out_channels, self.num_priors * 4,                **cfg.head_layer_params)
            self.conf_layer = conf_Conv2d(out_channels, self.num_priors * self.num_classes, **cfg.head_layer_params)
            self.mask_layer = mask_Conv2d(out_channels, self.num_priors * self.mask_dim,    **cfg.head_layer_params)
...
class FPN(ScriptModuleWrapper):
    ...
    def __init__(self, in_channels):
        ...
        """
        self.lat_layers  = nn.ModuleList([
            fpn_lat_layers_Conv2d(x, cfg.fpn.num_features, kernel_size=1)
            for x in reversed(in_channels)
        ])
        ...
        """
        self.pred_layers = nn.ModuleList([
            fpn_pred_layers_Conv2d(cfg.fpn.num_features, cfg.fpn.num_features, x, kernel_size=3, padding=padding)
        ...
        if cfg.fpn.use_conv_downsample:
            self.downsample_layers = nn.ModuleList([
                fpn_down_layers_Conv2d(cfg.fpn.num_features, cfg.fpn.num_features, x, kernel_size=3, padding=1, stride=2)
                for x in range(cfg.fpn.num_downsample)
            ])
        """    
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        """
        if cfg.use_semantic_segmentation_loss:
            self.semantic_seg_conv = seg_Conv2d(src_channels[0], cfg.num_classes-1, kernel_size=1)
    ...
    def save_weights(self, path):
        ...
        """
        if Yolact.evalbox > Yolact.maxbox or Yolact.evalmask > Yolact.maxmask:
            Yolact.maxbox = Yolact.evalbox
            Yolact.maxmask = Yolact.evalmask
