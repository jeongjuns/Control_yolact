...
from utils.functions import MovingAverage, make_net, SavePath
...
parser.add_argument('--save', default='/home/coki/yolact_pruned/importance/', type=str, metavar='PATH', help='path to save pruned model (default : pruned_weights/')
...
parser.add_argument('--start_iter', default=-1, type=int,
                    help='Resume training at this iter. If this is -1, the  will be'\
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                if flag==0:
                 im_score = set_list(net)
                 flag=1
                cnt=0
                check = 0
                for m in net.modules():
                    if (str(m).startswith('Sequential')) and (str(m).find('(10)')>0) and (str(m).find('Bottleneck')<0):
                        check=1
                    elif str(m).startswith('FPN'):
                        check=0
                    if check==1 and isinstance(m, nn.Conv2d):
                        im_score[cnt] += (m.weight*m.weight.grad).data.pow(2).view(m.weight.size(0),m.weight.size(1),-1).sum(dim=2)
                        cnt+=1
                cnt=0
...
print(args.ratio)
net_yolact = Yolact(path)
net_yolact.load_weights(path)
importance = train()
print('Finished')